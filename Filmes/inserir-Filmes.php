    ...
    if(isset($_FILES["txFileContent"])){
		...
		if($erro==0){
			if(is_uploaded_file($arqNomeTemp)){
				if(move_uploaded_file($arqNomeTemp,"Filmes/". $arqNome)){
					echo "Sucesso!";

					try {
		//Linha Conexão	
						$stmt = $pdo->prepare("INSERT INTO tbfilmes VALUES(null,'$nome','$categoria','$ano', '$diretor', '$duracao', '$classificacao','$sinopse','Filmes/$arqNome','$categoria2','$categoria3')");		
						$stmt->execute();				 				 
